...
package ArmRacos.Method;

import ArmMPC.Automata;
import ArmRacos.Tools.*;

import java.util.ArrayList;
import java.util.HashSet;

import ArmRacos.Componet.*;
import ArmRacos.ObjectiveFunction.*;
...
public class Continue extends BaseParameters{
	...
	private int pathChoice=39;

	private ArrayList<Automata> automatas;
	public HashSet<Integer> coverSet=new HashSet<>();
	public int simutime=0;
//	public HashMap<Integer, Double> eachBest =new HashMap<Integer, Double>();
//
//	public HashSet<Integer> badPoint=new HashSet<>();
...
//		for(int i=0;i<pathChoice*pathChoice;i++){
...
//							}
	...
	protected void Initialize(){
		...
		for(int i=0; i<SampleSize+PositiveNum; i++){
			...
			temp[i].setValue(val);
			coverSet.add((int)temp[i].getFeature(0));
			simutime++;
...
//			}
	...
	protected void ShrinkModel(Instance pos){
		ArrayList<Integer> vaildRange=task.getVaildRange(pos);
//		System.out.println("ShrinkModel");
		...
		while (ins_left>0) {//generate the model
			...
			++c;
			int index=ro.getInteger(0, vaildRange.size() - 1);
			ChoosenDim=vaildRange.get(index);
...
//				if(i%500==0){
//					for(int ii=0;ii<1521;ii++){
//						if(eachBest.get(ii)!=Double.MAX_VALUE && eachBest.get(ii)>1000*bestValue){
//							badPoint.add(ii);
//							System.out.println("******"+Integer.toString(ii)+" is a bad point******");
//						}
//					}
//				}
...
//						pathchoice2.add(NextPop[j].getFeature(1));
						if (notExistInstance(j, NextPop[j])) {
							double val=task.getValue(NextPop[j]);
							NextPop[j].setValue(val);//query

							reSample = false;
							coverSet.add((int)NextPop[j].getFeature(0));
							simutime++;
//							int ii=(int)NextPop[j].getFeature(0);
//
//							if(eachBest.get(ii)>val){
//								eachBest.put(ii,val);
//							}
...
*/
//		System.out.println("bad points set contains "+ Integer.toString(badPoint.size())+"/1521 choices");
		Mission of = (Mission) task;
		of.valueArc.iterativeNums = iterativeNums;
		of.valueArc.arrayListBestValues = arrayListBestValues;
		of.valueArc.pathinfo=new int[]{0,0,simutime,coverSet.size()};
		return of.valueArc;
	}
